...
/**
 * Servlet implementation class adminPurchasesList
 */
@WebServlet("/adminPurchasesList")
public class adminPurchasesList extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public adminPurchasesList() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<users> userList = uDAO.showUsers();
		request.setAttribute("userList", userList);
		
		List<shoes> shoeList = sDAO.showShoes();
		request.setAttribute("shoeList", shoeList);
		request.getRequestDispatcher("adminPurchases.jsp").forward(request, response);
